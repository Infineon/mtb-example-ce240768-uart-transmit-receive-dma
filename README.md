# PDL: SCB UART transmit-receive with DMA

This code example demonstrates the SCB UART transmit and receive operation using direct memory access (DMA). The application uses a serial terminal to read data and echoes what is received. UART is configured to perform both transmit and receive operations. 

[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce240768-uart-transmit-receive-dma)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[mtb-example-ce240768-scb-uart-transmit-and-receive-using-dma](https://github.com/Infineon/mtb-example-pdl-xmc7000-uart-transmit-receive-dma)" application with the desired name "ScbUartTransmitAndReceiveUsingDma" configured for the *KIT_T2G_C-2D-6M_LITE* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_T2G_C-2D-6M_LITE --app-id mtb-example-ce240768-pdl-scb-uart-transmit-and-receive-using-dma --user-app-name ScbUartTransmitAndReceiveUsingDma --target-dir "C:/mtb_projects"
   ```
...
4. After programming, the application starts automatically. Confirm that the UART terminal displays the message as shown in **Figure 1**.

    **Figure 1.  Terminal output on program startup**

   ![Terminal Output](images/output1.png)


5. After typing "a" on the terminal. The echo is shown in **Figure 2**.

    **Figure 2.  The echo on the terminal**

   ![Terminal Output](images/output2.png)
...
A UART block is configured to receive and send data to a terminal emulator. The application uses DMA to handle the data received in the UART Rx FIFO. Two DMA channels are used to handle the data in transmit and receive directions. On the receiving side, two SRAM buffers, known as 'ping-pong buffers,' are used alternately to store the data received from the UART terminal. This arrangement allows time to extract the data from either buffer.

The RxDma resource handles the data transfer in the receive direction. RxDma has two descriptors in the chain, configured to alternate the source between the ping-pong buffers in the receive direction.

The TxDma resource is used to handle data in the transmit direction. TxDma has a single descriptor configured to transfer one element at a time, alternating between the two ping-pong buffers. This data is finally echoed back as shown in **Figure 3**.
...
Resources  | Links
-----------|----------------------------------
Application notes  | [AN235305](https://www.infineon.com/assets/row/public/documents/10/42/infineon-an235305-getting-started-with-traveo-t2g-family-mcus-in-modustoolbox-applicationnotes-en.pdf) – Getting started with TRAVEO&trade; T2G family MCUs in ModusToolbox&trade;  <br>  [AN225401](https://www.infineon.com/assets/row/public/documents/10/42/infineon-an225401---how-to-use-serial-communication-block-scb-in-traveo-t2g-family-applicationnotes-en.pdf?fileId=8ac78c8c7cdc391c017d0d3e657867d2) – How to use serial communications block (SCB) in TRAVEO&trade; T2G family <br> [AN220191](https://www.infineon.com/assets/row/public/documents/10/42/infineon-an220191---how-to-use-direct-memory-access-dma-controller-in-traveot2g-family-applicationnotes-en.pdf) – How to use direct memory access DMA controller in TRAVEO&trade; T2G family <br>
...
Document title: *CE240768* – PDL: SCB UART transmit-receive with DMA* 
...
| 1.3.0   | Updated design.modus of the KIT_T2G_B-H-16M_LITE for addressing CLK warning and updated to support ModusToolbox&trade; v3.8 |
<br>
